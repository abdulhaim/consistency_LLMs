...
config_llm = {'agent1_model': 'Llama-3.1-8B-Instruct',
             'agent2_model': '/raid/users/ryan_cheng2/checkpoints/education/llama3-8b-kto-sft-large-prompt/',
...
write_file = os.path.join(output_dir, f"kto_sft_large_{config_llm['agent1_model']}_{config_llm['seed']}_{unique_num}.json")
...
if __name__ == "__main__":
  ...
  count = 0 
  for i in range(1):
      for topic, persona_item in tqdm(persona_final[:20]):
          count+=1
          print(count)
