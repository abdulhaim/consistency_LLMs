...
def generate_conversation(background_info_student, background_info_teacher, topic, pturn=1):
    '''
...
def main(argv):
    ...
    np.random.seed(0)
    random_indices = np.random.choice(len(conversation_prompts), size=3000, replace=False).astype(int).reshape(-1, 3) 
